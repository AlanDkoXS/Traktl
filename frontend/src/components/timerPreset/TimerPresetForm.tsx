...
export const TimerPresetForm = ({ timerPreset, isEditing = false }: TimerPresetFormProps) => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { createTimerPreset, updateTimerPreset } = useTimerPresetStore();

  const [name, setName] = useState(timerPreset?.name || '');
  const [workDuration, setWorkDuration] = useState(timerPreset?.workDuration || 25);
  const [breakDuration, setBreakDuration] = useState(timerPreset?.breakDuration || 5);
  const [repetitions, setRepetitions] = useState(timerPreset?.repetitions || 4);
