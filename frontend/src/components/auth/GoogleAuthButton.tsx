...
export const GoogleAuthButton = ({ isLogin = true }: GoogleAuthButtonProps) => {
  ...
  const handleGoogleResponse = async (response: any) => {
    console.log('Google response received with credential:', response.credential ? `${response.credential.substring(0, 20)}...` : 'none');
    try {
  ...
  return (
    <div className="w-full">
      ...
      <button
        type="button"
        className="w-full flex justify-center items-center gap-3 py-2.5 px-3 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-md transition-colors dark:bg-gray-800 dark:border-gray-600 dark:text-white dark:hover:bg-gray-700"
