import { TimeEntry, TimeEntryEntity } from '../../../domain/entities/time-entry.entity';
import { TimeEntryRepository } from '../../../domain/repositories/timeEntryRepository.interface';
import { ITimeEntry, TimeEntry as TimeEntryModel } from '../../../data/mongodb/models/timeEntry.model';
...
export class MongoTimeEntryRepository implements TimeEntryRepository {
   ...
   async update(id: string, timeEntry: Partial<TimeEntryEntity>): Promise<TimeEntry | null> {
   ...
   async listByUser(userId: string, page = 1, limit = 10): Promise<TimeEntry[]> {
   ...
   async listByProject(projectId: string, page = 1, limit = 10): Promise<TimeEntry[]> {
       const skip = (page - 1) * limit;
   ...
   async listByTask(taskId: string, page = 1, limit = 10): Promise<TimeEntry[]> {
       const skip = (page - 1) * limit;
   ...
   async findByCriteria(criteria: Partial<TimeEntryEntity>): Promise<TimeEntry[]> {
